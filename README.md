...
詳細な install 手順と日常運用は [§3 Quick Start](#3-quick-start) を参照。
...
## 0. はじめに — Claude Code が初めての方へ

このセクションは Claude Code を使ったことがない方向けの導入です。すでに概念をご存知の方は [§1](#1-完成形の配置構成) へ進んでください。

### 0.1 Claude Code とは

Claude Code は、ターミナル (黒い画面のコマンド入力環境) の中で動く AI コーディングアシスタント (AI 開発の相棒) です。あなたと対話しながら、PC 上のファイルを読み書きし、コマンドを実行し、コードを書く作業を代行します。「対話型 AI のコード版が、あなたの PC のファイルを直接触れるようになったもの」とイメージしてください。

### 0.2 このキット (engineer-claude-kit) は何をするか

会社環境 (AWS Bedrock + Windows) で Claude Code を使い始めるときに必要な **初期設定一式を 1 コマンドでまとめて用意** します。設定ファイルを手で書く手間と、覚えるべきことを減らすのが目的です。

具体的には、AI への共通の指示書、軽い作業を安いモデルへ自動で振り分ける仕組み、コミットメッセージの自動生成、機密情報の漏洩 (ろうえい) チェックなどを、まとめて配置します。

### 0.3 覚えておくと読みやすい用語

README の各所に出てくる用語を、平易な言い換えで先に示します。

| 用語 | 平易な言い換え | 補足 |
|---|---|---|
| エージェント (agent) | 作業役の AI | Claude 本体。あなたと対話する AI |
| サブエージェント (sub-agent) | 下請けの AI | 軽い作業を別の (多くは安い) AI に任せる仕組み |
| スキル (skill) | AI に足す「技能」 | 特定作業の手順書。例: テスト実行、ビルド操作 |
| スラッシュコマンド (slash command) | `/` で呼び出す機能 | 例: `/apply`、`/checkpoint` |
| CLAUDE.md | AI への指示書 | 「こう振る舞ってほしい」を書いたファイル。AI が起動時に必ず読む |
| settings.json | Claude Code の設定ファイル | 使うモデルや画面表示などの設定 |
| `~/` (チルダ・スラッシュ) | あなたのホームフォルダ | Windows では `C:\Users\<あなたのユーザー名>\`。PowerShell では `$env:USERPROFILE` |
| コンテキスト (context) | AI の作業メモリ | 会話が長くなると一杯になる。`/compact` で要約して空ける |
| statusLine | 画面下部の情報バー | 現在のモデル名やメモリ使用率を表示する 1 行 |
| ADR (Architecture Decision Record) | 設計の決定理由メモ | 「なぜこう作ったか」の記録。`docs/adr/` にある |
| bootstrap / apply | 初回配置 / 更新反映 | bootstrap = 初回の一括設置、apply = その後の更新反映 |
| SSoT (Single Source of Truth) | 大元 (おおもと) のファイル | ここを直して各所へ配る。配布先を直接編集しない |

> **パス表記について**: 本 README の説明文では、Claude Code 公式ドキュメントに合わせて `~/.claude/` のような Unix 風表記を使います。Windows では `~/` を **`C:\Users\<あなたのユーザー名>\`** に読み替えてください (例: `~/.claude/settings.json` = `C:\Users\<あなたのユーザー名>\.claude\settings.json`)。コマンド例 (PowerShell) では同じ場所を `$env:USERPROFILE\.claude\` と書いています。

### 0.4 まずこれだけ

1. このリポジトリを clone して `bootstrap.ps1` を実行する ([§3.1](#31-初回-install-claude-code-を起動できる状態にする))
2. Claude Code を起動する — これでキットの機能が使える状態になる
3. 以降のキット更新の反映は、Claude Code の中で `/apply` を打つだけ ([§3.2](#32-日常運用-claude-code-内で))

以下 [§1](#1-完成形の配置構成) からは、どこに何が配置され、各ファイルが何のためにあるかを詳しく説明します。最初は読み飛ばして [§3 Quick Start](#3-quick-start) から始めても構いません。

## 1. 完成形の配置構成

**目的**: engineer-claude-kit が完成時にどこへ何を配置するかを 3 視点 (グローバル / プロジェクト / SSoT) で示す。bootstrap 実行後の状態を理解する起点。

配布の全体像 (SSoT → 配布先) は以下のとおり。kit リポジトリ (SSoT) の `templates/` / `source/` を編集し、`bootstrap.ps1` (初回) と `/apply` → `apply-claude-kit.ps1` (以降) が `~/.claude/` (グローバル) と `<project>/.claude/` (プロジェクト) へ配布する。

```mermaid
flowchart LR
    user([ユーザ]) -->|初回 1 回| boot[bootstrap.ps1]
    user -->|以降 Claude Code 内| apply["/apply (apply-claude-kit.ps1)"]
    subgraph SSoT["engineer-claude-kit (SSoT)"]
        tmpl[templates/]
        src[source/rules/]
        cfg[config/*.yaml]
    end
    boot --> SSoT
    apply --> SSoT
    SSoT -->|配布| global["~/.claude/ (グローバル §1.1)"]
    SSoT -->|配布| proj["&lt;project&gt;/.claude/ (プロジェクト §1.2)"]
```
...
|   |-- leak-check/           # PII / credentials 検出 (5 層防御 / leak protection governance は ADR-0009 で確定)
|   |-- commit-helper/        # Conventional Commits 補助
|   |-- propose-adr/          # ADR draft 起票 (architect sub-agent 連携)
|   |-- android-build/        # Android ビルド・ADB 操作支援 (project-recommend 候補)
|   |-- web-test/             # Web/Node test 実行・依存管理支援 (project-recommend 候補)
|   |-- python-test/          # Python pytest・venv 管理支援 (project-recommend 候補)
|   |-- skill-installer/      # Global skill → project コピー helper (/install-skill 連携)
|   `-- cleanup-orphan-processes/  # 孤立 bash/gh/git subprocess 掃除 (ADR-0011)
...
凡例: ✅ 実装済 / ⏳ 計画 (未着手 または 配布機能未完成)。⏳ には「実体ファイルが kit リポジトリ内に存在するが、ユーザ環境への配布機能 (bootstrap.ps1 / apply-claude-kit.ps1) がまだ完成していない」ケースを含む。Phase 1 (foundation) + Phase 2 (bootstrap / apply / model 配布) + Phase 3.1 (共通 skills) + Phase 3.2 (Bedrock cost 観測) + Phase 4 系 (ADO CI / project-recommend skills / work-end-reminder / repository governance) が実装済 (Pester テスト群は PS 5.1 / Pester 3.4 互換、手動検証ドキュメントも整備済)。下表で ⏳ が残るのはプロジェクト側 `.claude/skills/` 等の **配布機能未完成** 項目のみ。
...
| ADR | テーマ | ステータス |
|---|---|---|
| [ADR-0001](docs/adr/0001-clean-start-design.md) | clean start 設計 (モデル戦略 / persona / host / OS) | Accepted |
| [ADR-0002](docs/adr/0002-adr-curation-policy.md) | ADR セット取捨選択方針 (A/B/C/D 分類) | Accepted |
| [ADR-0003](docs/adr/0003-bootstrap-and-abstraction.md) | bootstrap design + `config/*.yaml` SSoT | Accepted |
| [ADR-0004](docs/adr/0004-claude-md-auto-model-routing.md) | CLAUDE.md による Haiku/Sonnet 4.5 自動使い分け | Accepted |
