...
### 出力形式: 技術メトリクス + 人間語併記 (G6g follow-up)

技術値だけだと engineer 以外には伝わらず、人間語だけだと事実検証ができない。
両方を併記し、user が「事実 (数値)」と「意味 (対人類比)」の両面で理解できる構造にする。
各 finding の技術メトリクス行は維持し、その直後に blockquote で平易な言い換えを **追記**
(置換ではない)。条件成立時のみ付与し、健全な指標には付けない。

例:

> ### Opus が総 cost の 99.9%
> - input/output token: 5000 / ...
>
> > 複雑じゃない仕事まで一番頭のいい人 (Opus) に任せている状態。
> > 簡単な作業は別の人 (Haiku) に振ると速くて安く済みます。

対人組織での「report と一言コメント」の関係に近い。

#### 日本語原文の配置 (ASCII-only script との両立)

本 kit は PowerShell source を **ASCII-only** に固定し (ADR-0003 §C)、PS 5.1 は
BOM 無し `.ps1` を system ANSI codepage (CP932) で parse するため、日本語を `.ps1`
へ直書きすると parse 時に mojibake する。よって人間語原文は
`scripts/lib/plain-language-hints.json` (UTF-8、no BOM) に外出しし、`Read-Utf8NoBom`
(codepage 非依存の強制 UTF-8 decode) 経由で読む。renderer `Get-PlainLanguageHint`
(`scripts/lib/plain-language.ps1`) は ASCII-only を維持。これにより encoding を
単一系統 (UTF-8 no BOM + 強制 decode) に保ったまま日本語出力を実現する。
...
### kit-behind 検出の統合 (G6h)

usage-insights 自体が scheduled-task で日次/週次に動くため、ここに `Test-KitBehind`
(ADR-0013) の呼出を統合する。

- 利点: kit 更新気づきを `apply` 明示実行に依存しなくなる
- 実装: `scripts/usage-insights.ps1` が `scripts/lib/kit-updater.ps1` を dot-source、
  behind > 0 の時のみレポート冒頭に「kit 更新あり」セクションを追記
- legacy 互換: `kit-updater.ps1` 不在 (G6b 以前 install) は silent skip
- network 失敗: Test-KitBehind が -1 返し、レポート未追記
- 純粋性の維持: `Test-KitBehind` 呼出は `Invoke-UsageInsights` (main) に置き、
  `Format-InsightsReport` は `-KitBehind <int>` を受け取るだけの純関数とする。これにより
  既存の Format-InsightsReport テスト (blockquote 0 件を assert する healthy ケース等) が
  network 状態に依存せず決定論を保つ。banner 描画は `Format-KitBehindBanner`
  (`scripts/lib/plain-language.ps1`) へ抽出し、人間語は `KitBehind` category として
  `plain-language-hints.json` に外出し ({Behind} placeholder を `Get-PlainLanguageHint`
  の -Metrics で置換)
