...
|---|---|---|
| 0017 | cost observation kit (週次観測) | コスト観測は ADR-0001 の中核関心だが、観測基盤が scheduled-tasks 依存 (§F で削除)。Bedrock cost 観測の代替手段を Phase 3 で再評価 |
...
1. D 保留 3 件 (0017 cost / 0019 native-tech / 0034 upgrade) の再評価タイミング —
   いずれも Phase 2 の apply-claude-kit 実装完了後が妥当か
2. B 変形移植 18 件の移植先 ADR 番号採番方針 (1 ADR=1 番号 か、関連 ADR を統合採番か)
...
- engineer-claude-kit の Phase 2 以降の実装範囲が A/B 25 件に明確化
...
- B 変形移植 18 件の具体内容は移植実装時に再度設計が必要 (本 ADR は範囲確定のみ)
