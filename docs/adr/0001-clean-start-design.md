...
職場環境 (AWS Bedrock + Azure DevOps + Windows) で Claude Code を使い始める
エンジニア向けに、初期セットアップの認知負荷を最小化する self-contained な
bootstrap キットを設計する。対象とする前提・要件は以下:
...
汎用の高機能テンプレートではなく、業務環境で単独展開できる self-contained な
構成を優先する。Layer 1 骨格 (5 層防御の思想) と ADR 文化を採用し、コード資産は
**新規実装** する。

### 前提: モデル戦略の根拠 (Bedrock TTL 実測検証)

実環境での実測 (2026-06-04、Bedrock 経由 + 社内 Cloud Proxy SSL
inspection) の核心発見:
...
(上記コスト数値・model ID は実環境での Bedrock 1h cache 実測検証に基づく)
...
- **Sonnet 4.6 / Opus 系: 採用しない** (Sonnet 4.6 は Bedrock 1h cache 非対応で
  TTL 5min 短期化、cache miss コストが嵩む。実測検証に基づく)
...
**注記** (2026-06-05): 上記 model ID は Bedrock 実測値 (実環境での 1h cache
実測検証 §3.1 / §8.1 相当) に
...
配布元 URL は `ENGINEER_CLAUDE_KIT_GIT_URL` 環境変数で override する設計とし、ADR / コードを変更せずに組織移行・リポ rename に追従できるようにする。

**出典 (Phase 1 実測値の根拠)**: 実環境での Bedrock 1h cache 実測検証 §3.1 / §8.1 (Sonnet 4.5 / Haiku 4.5 の Bedrock model ID 実測)
...
### 代替案 1: 既存の高機能テンプレートリポを fork / 派生

- メリット: ADR 蓄積を直接流用、CI 流用
- デメリット: 機能過多が初心者に重い、git 履歴に不要 ADR が永久残存

### 代替案 2: 既存テンプレートリポ内に `--profile minimal` を追加
...
- デメリット: Haiku 4.5 の Bedrock 1h cache 対応は実測検証 (Phase 1) で
...
7. ADR セットの取捨選択 (どの設計判断を採録し、どれを破棄するか) の精査リスト (別途 ADR-0002 で決定)
8. `cloud-build-wrapper` を採用するか (OneDrive 業務利用が職場で多いか要確認)
...
- Layer 1 骨格 (5 層防御の思想、ADR 文化) を最小構成として採用

### 欠点

- 高機能テンプレートの蓄積資産を直接流用しないため、必要機能は都度新規実装する必要がある
- ADR 蓄積をゼロから書き起こすコスト (重要な設計判断の採録が次の課題)
...
- ADR-0003 (bootstrap 設計 + モデル抽象化)
- ADR-0004 (CLAUDE.md による Haiku / Sonnet 自動使い分け)
- モデル戦略の根拠は実環境での Bedrock 1h cache 実測検証 (§G / §G-2) に基づく
- bootstrap.ps1 / apply-claude-kit.ps1 の β 配布方式 (§B)
